function [ mD, mA, vDicErr, vLambdaFctr ] = ApplyDynamicRlsDla( mX, mDInit, lambdaFctr, ompThr, ompMode, dicErrThr )
% ----------------------------------------------------------------------------------------------- %
...
lambdaStepSize = (3 * (1 - lambdaFctr)) / numSamples;

vLambdaFctr = nan([(numSamples + 1), 1]);
...
for ii = 1 : numSamples
    ...
    effResNorm = max(norm(vR), 0.01);
    lambdaFctr = lambdaFctr + ((1 / effResNorm) * lambdaStepSize);
    lambdaFctr = min(lambdaFctr, 1);

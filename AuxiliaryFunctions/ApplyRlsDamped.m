function [ vEstParams, mEstCov ] = ApplyRlsDamped( vMeasurements, mModel, dampingFactor, vEstParams, mEstCov )
% ----------------------------------------------------------------------------------------------- %
...
mI              = eye(modelOrder);
dampingFctrInv  = 1 / dampingFactor;

for iIter = 1:numIterations
    vModelRow   = mModel(iIter, :);
    vGainFactor = (mEstCov * vModelRow.') / (dampingFctrInv + (vModelRow * mEstCov * vModelRow.'));
    vEstParams  = vEstParams + (vGainFactor * (vMeasurements(iIter) - (vModelRow * vEstParams)));
    mEstCov     = dampingFctrInv * (mI - (vGainFactor * vModelRow)) * mEstCov;
end
